...
export const GET_ASSOSIATION = gql`
    query getAssosiation($uuid: ID!) {
        assosiation(web_user_uuid: $uuid) {
            web_user_uuid
            assn_name
            manager
            address
...
export const UPDATE_ASSN_INFO = gql`
    mutation updateAssnInfo(
        $uuid: ID!
        $assn_name: String!,
        $manager: String!,
        $address: String!,
...
function Content(props) {
  ...
  const { data, loading, error } = useQuery(GET_ASSOSIATION, 
    { variables : { uuid : JSON.parse(Cookies.get("signIn")).uuid } }, );

    const[ updateAssnInfo ] = useMutation(UPDATE_ASSN_INFO, {
      onCompleted({updateAssnInfo}) {
        window.location.reload();
  
        // console.log(updateAssnInfo);
      }
    })
    ...
    event.preventDefault();
      ...
      updateAssnInfo({
        variables: {
          ...
          assn_name: event.target.assn_name.value,
        ...
        <form className={classes.inputForm} onSubmit={onSubmit}>
            <div className={classes.inputContainer}>
                ...
                <TextField 
                  ...
                  variant="outlined" 
                  margin="normal" 
                  defaultValue={data && data.assosiation.assn_name} 
                />
            </div>
            <div className={classes.inputContainer}>
                ...
                <TextField 
                  ...
                  variant="outlined" 
                  margin="normal" 
                  defaultValue={data && data.assosiation.manager} 
                />
            </div>
            <div className={classes.inputContainer}>
                ...
                <TextField 
                  ...
                  variant="outlined" 
                  margin="normal" 
                  defaultValue={data && data.assosiation.phone} 
                />
            </div>
            <div className={classes.inputContainer}>
                ...
                <TextField 
                  ...
                  variant="outlined" 
                  margin="normal" 
                  defaultValue={data && data.assosiation.address} 
                />
            </div>
            <div className={classes.inputContainer}>
                ...
                <TextField 
                  ...
                  multiline rows="6" 
                  margin="normal" 
                  defaultValue={data && data.assosiation.description} 
                />
            </div>
            <div className={classes.inputContainer}>
                <Typography className={classes.inputLabel}>협회 사진<br/>(최대 5장)</Typography>
                <div className={classes.inputArea}>
                  {
                    !isFileChanged && (
                      <div className={classes.imgArea}>
                      <GridList className={classes.gridList} cellHeight={205}>
                        {data && data.assosiation.images.map(img => (
