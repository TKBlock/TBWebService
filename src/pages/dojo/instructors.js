...
const GET_REQUESTED_USER = gql`
  query requestedUser($uuid: ID!) {
    registratedInstructor(uuid: $uuid, state: 1) {
      mobile_user_uuid
      name
      age
      address
...
const GET_APPROVEED_USER = gql`
  query approvedUser($uuid: ID!) {
    registratedInstructor(uuid: $uuid, state: 2) {
      mobile_user_uuid
      name
      age
      address
...
const UPDATE_REG_STATE = gql`
  mutation updateRegistratedInstructorState(
...
const DELETE_REG = gql`
  mutation deleteRegistrateInstructor(
...
function Content(props) {
  ...
  const [ updateRegistratedInstructorState ] = useMutation(UPDATE_REG_STATE, {
    onCompleted({updateRegistratedInstructorState}) {
  ...
  const [ deleteRegistrate ] = useMutation(DELETE_REG, {
    onCompleted({deleteRegistrateInstructor}) {
      // window.location.reload();
          ...
          <Paper className={classes.paper}>
            <MuiThemeProvider theme={theme}>
              <MaterialTable
                  ...
                  data={aprResponse.loading ? [] : aprResponse.data.registratedInstructor}
                  options={{
                    pageSize: 10,
                    selection: true
                  }}
          ...
          <Paper className={classes.paper}>
            <MuiThemeProvider theme={theme}>
              <MaterialTable
                ...
                data={regResponse.loading ? [] : regResponse.data.registratedInstructor}
                options={{
                  pageSize: 10,
                  selection: true
                }}
